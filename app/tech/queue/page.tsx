...
export default async function TechQueuePage() {
  ...
  const { data: rows, error } = await sb
    ...
    .select('id,status,vehicle_id,service,fmc,mileage,started_at')
    .in('status', ['IN_PROGRESS','RESCHEDULED'])
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold">Tech • Active</h1>
      {!rows?.length && <div className="text-sm opacity-70">No active requests.</div>}
        ...
        {rows?.map((r) => (
          <div key={r.id} className="rounded-2xl border p-4 shadow-sm bg-white">
            ...
            <div className="flex gap-2 mb-3">
              <RequestAction id={r.id} to="COMPLETED" label="Complete" />
              <RequestAction id={r.id} to="RESCHEDULED" label="Reschedule" />
            </div>
