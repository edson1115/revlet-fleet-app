      ...
      <div className="overflow-x-auto border rounded">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="text-left p-3">Started/Created</th>
            ...
            {(rows ?? []).map((r: any) => {
              ...
              return (
                <tr key={r.id} className="border-t align-top">
                  ...
                  <td className="p-3"><span className="inline-block rounded-full text-xs px-2 py-1 bg-green-50 border border-green-200">{r.status}</span></td>
                  <td className="p-3">
                    <form action={completeAction}><input type="hidden" name="id" value={r.id} />
                      <button className="px-3 py-1 rounded bg-black text-white hover:opacity-80">Complete</button>
                    </form>
                  </td>
                </tr>
              );
            })}
            {(rows ?? []).length === 0 && <tr><td colSpan={8} className="p-6 text-center text-gray-500">No IN_PROGRESS.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
