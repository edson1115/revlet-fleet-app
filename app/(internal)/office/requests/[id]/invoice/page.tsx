...
import StripePaymentModal from "./StripePaymentModal"; // 👈 Import the Modal
...
const IconCard = () => <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>;

export default function InvoiceBuilderClient({ request }: { request: any }) {
  const router = useRouter();
  const [processing, setProcessing] = useState(false);
  const [showStripe, setShowStripe] = useState(false); // 👈 Toggle for Modal
  ...
  // --- 2. HANDLE PAY ---
  // Reusable function to close ticket (Called by Manual OR Stripe)
  async function handleFinalize(method: "MANUAL" | "STRIPE" = "MANUAL") {
    setProcessing(true);
    
    // Call the API route we created to handle the DB update
    ...
    if(res.ok) {
        alert(method === "STRIPE" ? "✅ Payment Successful! Ticket Closed." : "✅ Invoice Marked Paid & Closed!");
    ...
    <div className="min-h-screen bg-gray-100 p-8 font-sans">
      ...
      {showStripe && (
        <StripePaymentModal 
            ...
            onSuccess={() => {
                setShowStripe(false);
                handleFinalize("STRIPE"); // 👈 Triggers the DB update
            }} 
        />
      )}
         ...
         <div className="flex gap-3">
             ...
             {request.status !== "BILLED" && (
                 <>
                    ...
                    <button 
                        onClick={() => {
                            if(confirm("Confirm manual cash/check payment?")) handleFinalize("MANUAL");
                        }}
                        disabled={processing}
                        className="bg-zinc-800 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-black flex items-center gap-2"
                     >
                        <IconCheck /> Manual Pay
                     </button>
                     ...
                     <button 
                        ...
                        disabled={processing}
                        className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-bold shadow-lg hover:bg-blue-700 flex items-center gap-2"
                     >
                        <IconCard /> Pay Card ${grandTotal.toFixed(2)}
                     </button>
                 </>
             )}
         </div>
