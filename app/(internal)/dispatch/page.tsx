...
import { setSchedule } from "@/lib/scheduling/setSchedule";
...
export default function DispatchPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (ids.length) {
      const thumbs = await getJSON<any>(
        `/api/images/list?request_ids=${encodeURIComponent(ids.join(","))}`
      );
  ...
  const grouped = useMemo(() => {
    const norm = (s: string) =>
      String(s || "").toUpperCase().replace(/_/g, " ");

    const waiting = rows.filter(
      (r) => norm(r.status) === "WAITING TO BE SCHEDULED"
    );

    const needs = rows.filter(
      (r) =>
        norm(r.status) === "RESCHEDULE" ||
        (r.dispatch_notes || "").toLowerCase().startsWith("tech send-back:")
    );

    const scheduled = rows.filter((r) => norm(r.status) === "SCHEDULED");
    const inProgress = rows.filter((r) => norm(r.status) === "IN PROGRESS");
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1">
            <div className="text-xs text-gray-700 font-medium">
              {fmtWindow(r.scheduled_at, r.scheduled_end_at)}
            </div>
  ...
  return (
    <div className="px-6 py-10 space-y-10">
      ...
      {/* WAITING */}
      <div>
        <h2 className="font-semibold text-lg mb-3">Waiting to Schedule</h2>
                  ...
                  try {
                    const resp = await fetch(
                      `/api/requests/${drawerData.id}`,
                      {
                        ...
                        body: JSON.stringify({ status: "RESCHEDULE" }),
                      }
                    );
