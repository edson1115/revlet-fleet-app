...
export default function DispatchScheduledPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-semibold mb-4">Scheduled</h1>
      {loading ? <p>Loading…</p> : rows.length === 0 ? <p>No scheduled jobs.</p> : (
          ...
          {rows.map((r) => (
            <li key={r.id} className="border rounded p-3 flex items-center justify-between">
              ...
              <button
                ...
                onClick={async () => {
                  const res = await fetch(`/api/requests/${r.id}/start`, { method: "PATCH" });
