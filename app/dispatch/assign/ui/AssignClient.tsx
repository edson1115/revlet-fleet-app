...
export default function AssignClient() {
  ...
  const [prevEtaStart, setPrevEtaStart] = useState<string | null>(null);
  const [prevEtaEnd, setPrevEtaEnd] = useState<string | null>(null);
  ...
  // Load requestId from query param
  useEffect(() => {
  ...
  // Load techs + existing request info
  useEffect(() => {
    (async () => {
      try {
        setErr("");
        ...
        // Load tech list
        const t = await getJSON<{ techs?: Tech[]; rows?: Tech[]; options?: Tech[] }>(
          "/api/lookups?scope=techs"
        );
        ...
        if (requestId) {
          ...
          // ETA
          if (r?.eta_start) {
            setPrevEtaStart(r.eta_start);
            setEtaStart(toLocalInputValue(new Date(r.eta_start)));
          }

          if (r?.eta_end) {
            setPrevEtaEnd(r.eta_end);
            setEtaEnd(toLocalInputValue(new Date(r.eta_end)));
          }
  ...
  async function save() {
    ...
    try {
      ...
      // Include ETA if filled
      if (etaStart) body.eta_start = new Date(etaStart).toISOString();
      if (etaEnd) body.eta_end = new Date(etaEnd).toISOString();

      await patchJSON(`/api/requests/${requestId}/schedule`, body);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4 text-sm">
        <Link className="text-blue-600 underline" href="/dispatch/scheduled">
          Back to Dispatch
        </Link>
        <Link className="text-blue-600 underline" href="/office/queue">
          Office Queue
        </Link>
      </div>
      ...
      <div className="rounded-2xl border p-4 space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            />
            <div className="text-xs text-gray-500 mt-1">
              Previously: {prevEtaStart ? new Date(prevEtaStart).toLocaleString() : "Not set"}
            </div>
          </div>
          ...
          <div>
            ...
            />
            <div className="text-xs text-gray-500 mt-1">
              Previously: {prevEtaEnd ? new Date(prevEtaEnd).toLocaleString() : "Not set"}
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => history.back()} className="rounded-xl border px-4 py-2">
            Cancel
          </button>
          <button onClick={save} className="rounded-xl bg-blue-600 text-white px-4 py-2">
            Save Assignment
          </button>
        </div>
      </div>
