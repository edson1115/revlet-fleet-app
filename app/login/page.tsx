...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // 2) Ensure we have tokens to sync to server cookies
    ...
    // 3) Sync session to SERVER cookies via your route handler
    //    This is the key fix for Vercel/SSR layouts that rely on cookies.
    try {
      ...
      if (!resp.ok || !result?.success || !result?.redirectUrl) {
        console.error("set-session failed:", result);
        setError(result?.error || "Failed to sync session. Please try again.");
        setLoading(false);
        return;
      }

      // 4) Hard redirect so Server Components read fresh cookies immediately
      setLoading(false);
      window.location.replace(result.redirectUrl as string);
      return;
    } catch (err) {
      console.error("set-session fatal:", err);
      setError("Login succeeded, but failed to establish a server session.");
      setLoading(false);
      return;
    }
  };
    ...
    <div className="min-h-screen bg-[#0B0F19] text-white font-sans flex overflow-hidden">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#0F172A] relative flex-col justify-between p-12 border-r border-white/5">
        ...
        <div className="relative z-10">
          <div
            onClick={() => router.push("/")}
            className="text-2xl font-black italic tracking-tighter flex items-center gap-2 cursor-pointer mb-2"
          >
        ...
        <div className="w-full max-w-[400px]">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              type="button"
              disabled
              className="w-full bg-slate-900 border border-slate-700 text-slate-400 font-medium h-12 rounded-lg flex items-center justify-center gap-2 hover:bg-slate-800 transition cursor-not-allowed opacity-60"
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                ...
                />
                <path
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.52 1 3.8 3.55 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                  fill="#34A853"
                />
                <path
                  d="M12 23c2.97 0 5.46-1.01 7.28-2.69l-3.54-2.87c-.99.66-2.23 1.06-3.74 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.8 20.45 7.52 23 12 23z"
                  fill="#34A853"
                />
                <path
                  d="M23 12c0-.66-.12-1.32-.27-1.96H12v3.75h6.35c-.53 2.57-2.73 4.26-5.35 4.26-2.5 0-4.63-1.5-5.71-3.75l-3.02 2.44C6.03 19.66 9.35 21.38 13.07 21.38c5.44 0 9.87-3.93 10.61-9.38H23z"
                  fill="#4285F4"
                />
              </svg>
              Sign in with SSO
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-slate-500">
                ...
                <Link href="/signup" className="text-blue-500 font-bold hover:text-blue-400">
                  Sign up here
                </Link>
              </p>
            </div>
