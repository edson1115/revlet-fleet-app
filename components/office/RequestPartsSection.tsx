...
export function RequestPartsSection({ 
  ...
  async function applyPackage(pkgId: string) {
      ...
      if (vehicleContext) {
          ...
          const vehicleMake = (vehicleContext.make || "").toLowerCase();
          const commonMakes = ["ford", "chevrolet", "chevy", "honda", "toyota", "nissan", "dodge", "jeep"];
          const detectedMake = commonMakes.find(m => pkgName.includes(m));

          if (detectedMake && !vehicleMake.includes(detectedMake)) {
              if (!confirm(`⚠️ COMPATIBILITY WARNING ⚠️\n\nApply '${selectedPkg.name}' to a ${vehicleContext.year} ${vehicleContext.make}?`)) return;
          } else {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
       ...
       <div className="flex justify-between items-center mb-4">
          <h3 className="text-sm font-bold uppercase tracking-wide text-gray-900">Parts & Supplies</h3>
          
          <div className="flex gap-2">
            <button 
                onClick={openPackageModal}
                className="flex items-center gap-2 px-3 py-1.5 bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200 rounded-lg text-xs font-bold transition"
            >
                <IconPackage /> Load Bundle
            </button>

            <button 
                onClick={handleConsultAI}
                disabled={aiLoading}
                className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 rounded-lg text-xs font-bold transition disabled:opacity-50"
            >
       ...
       {aiSuggestions.length > 0 && (
           <div className="mb-6 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 rounded-xl p-4 animate-in fade-in slide-in-from-top-2">
               <div className="flex justify-between items-center mb-3">
                   <h4 className="text-xs font-bold text-indigo-900 uppercase flex items-center gap-2">
                       <IconSparkles /> Recommended for {vehicleContext?.model}
                   </h4>
                   <button onClick={() => setAiSuggestions([])} className="text-xs text-indigo-400 hover:text-indigo-800">Close AI</button>
               </div>
               <div className="space-y-2">
                   {aiSuggestions.map((p, i) => (
                       <div key={i} className="flex items-center justify-between bg-white/80 p-2.5 rounded-lg border border-indigo-50 shadow-sm hover:border-indigo-200 transition">
                           ...
                           <button 
                               onClick={() => {
                                   setDraftPart({ inventory_id: "", part_name: p.name, part_number: p.number, quantity: p.qty });
       ...
       {isAdding && (
           <div className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-xl relative animate-in fade-in slide-in-from-top-2">
               <label className="text-[10px] font-bold text-gray-500 uppercase mb-1 block">Search Inventory or Type Custom</label>
               ...
               {!draftPart.part_name ? (
                               ...
                               {searchResults.map(item => (
                                   ...
                                   >
                                       ...
                                       <div className="flex justify-between text-xs text-gray-500 mt-0.5">
                                            ...
                                            <span className={item.quantity > 0 ? "text-green-600 font-bold" : "text-red-500 font-bold"}>
                   ...
                   <div className="flex justify-between items-center bg-white p-3 border border-green-200 rounded-lg mb-2 shadow-sm">
                       ...
                       <button 
                         onClick={() => setDraftPart({ inventory_id: "", part_name: "", part_number: "", quantity: 1 })}
               ...
               <div className="flex gap-3 mt-3">
                   ...
                   <div className="flex-1 flex items-end justify-end gap-2">
                       <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm text-gray-500 font-bold hover:text-black transition">Cancel</button>
                       <button onClick={handleAddPart} className="px-6 py-2 bg-black text-white text-sm font-bold rounded-lg shadow-lg hover:bg-gray-800 transition">Save Part</button>
                   </div>
               </div>
           </div>
       )}
       ...
       {/* LIST */}
       {loading ? (
           <div className="text-sm text-gray-400 animate-pulse">Loading parts...</div>
           ...
           <div className="space-y-2">
               {parts.map((part) => (
                   <div key={part.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border border-gray-100 group hover:border-gray-300 transition">
                       ...
                       {/* ACTIONS (Hover to see) */}
                       <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                            <button 
                                onClick={() => { setEditingPart(part); setShowEditModal(true); }}
                   ...
                   <div className="p-4 max-h-[60vh] overflow-y-auto space-y-3">
                       {packages.map(pkg => (
                           <div 
                                ...
                                className="border border-gray-200 p-4 rounded-xl hover:border-purple-400 hover:shadow-md cursor-pointer transition group"
                           >
                               <div className="flex justify-between items-start mb-1">
                                   <div className="font-bold text-gray-900">{pkg.name}</div>
                                   <div className="text-xs bg-gray-100 px-2 py-1 rounded font-bold text-gray-500 group-hover:bg-purple-50 group-hover:text-purple-600 transition">
                       ...
                       {packages.length === 0 && (
                           <div className="text-center py-10 text-gray-400 italic">No packages found.</div>
                       )}
                   </div>
               </div>
           </div>
       ...
       {showEditModal && editingPart && (
           <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
               <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
                   <h3 className="font-bold text-gray-900 mb-4">Edit Line Item</h3>
                   ...
                   <div className="flex gap-2 mt-6">
                       ...
                       <button 
                          ...
                          className="flex-1 py-3 text-sm font-bold text-white bg-black rounded-xl hover:bg-gray-800"
                       >
                           Save Changes
                       </button>
                   </div>
